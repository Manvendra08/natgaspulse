...
function toAuthErrorMessage(err: any): string {
    const message = String(err?.message || err?.msg || 'Login failed');
    if (/unsupported provider/i.test(message)) {
        return 'Google login is not enabled in your Supabase project. Enable Google under Authentication > Providers.';
    }
    ...
    if (/email not confirmed/i.test(message) || /email_not_confirmed/i.test(message)) {
        return 'Email not verified. Please verify your email first, or resend verification email below.';
    }
    return message;
}
...
export default function LoginPage() {
    ...
    const signInWithEmail = async () => {
        ...
        try {
            ...
            setError(msg);
            if (/not verified|email not verified|email not confirmed/i.test(msg)) {
                setCanResend(true);
            }
        } finally {
            setLoading(false);
            ...
            <div className="flex-1 flex items-center justify-center p-6">
                <div className="w-full max-w-md bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-7 shadow-2xl">
                    <div className="flex items-center gap-3 mb-6">
                        ...
                        <div>
                            <h1 className="text-2xl font-black text-zinc-900 dark:text-zinc-100">Log In</h1>
                            <p className="text-sm text-zinc-500">Access the full dashboard</p>
                        </div>
                    </div>
                    ...
                    <div className="space-y-3">
                        ...
                        <button
                            ...
                            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-black transition disabled:opacity-50"
                        >
                            Log In
                    ...
                    <div className="mt-5 text-xs text-zinc-500 flex items-center justify-between">
                        <span>
                            New here?{' '}
                            <a className="font-bold text-emerald-600 dark:text-emerald-400 hover:underline" href={`/signup?redirect=${encodeURIComponent(redirectTo)}`}>
                                Create an account
                            </a>
                        </span>
                        <a className="font-bold text-violet-600 dark:text-violet-400 hover:underline" href="/forgot">
                            Forgot password?
                        </a>
                    </div>
                </div>
